...
2. **Generate client-specific Page Creator Skill (`.agent/skills/[client-slug]-pages/SKILL.md`)**:
   ...
   - **Semantic FAQ Rule**: Banish generic placeholder or guessed questions. The FAQ section must always be derived from real Google 'People Also Asked' (PAA) questions for the page's target keywords. Use the Data For SEO tool (`serp_organic_live_advanced`) to retrieve real-time PAA questions. If Data For SEO is unavailable, execute web search queries targeting UK PAA data.
      - **Keyword & Client Focus**: Each FAQ question must target either the page's primary keyword or a secondary keyword. Answers must represent the client's business directly (e.g. from the contractor's perspective: "We are NICEIC-approved...", "Our local team...") with the primary topic and local intent of the page in mind.
      - **Sensitive Pricing Rule**: Never guess or estimate sensitive information such as pricing. If pricing is not explicitly documented elsewhere in the public site content, the agent must ask the user for exact numbers or omit/reframe the pricing FAQ to focus on scope and fixed-quotes.
