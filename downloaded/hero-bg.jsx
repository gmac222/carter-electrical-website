...
function HeroCanvas() {
  ...
  React.useEffect(() => {
    ...
    const addNode = (c, r) => {
      const k = key(c, r);
      if (!nodes.has(k)) nodes.set(k, { c, r, pulse: 0 });
      return nodes.get(k);
    };
    ...
    function build() {
      ...
      for (let i = 0; i < numTraces; i++) {
        ...
        if (startEdge < 0.5) {
          ...
          // from top
          c = Math.floor(Math.random() * cols); r = 0;
          dir = [0, 1];
        }
        const path = [[c, r]];
        addNode(c, r);
        ...
        for (let step = 0; step < maxLen; step++) {
          ...
          if (c < -1 || r < -1 || c > cols || r > rows) break;
          // Turn 90deg
          if (dir[0] !== 0) dir = [0, Math.random() < 0.5 ? 1 : -1];
          else dir = [Math.random() < 0.5 ? 1 : -1, 0];
        }
    ...
    function spawnPulse(now) {
      ...
      pulses.push({
        trace: t,
        t: 0,
        speed: 0.00022 + Math.random() * 0.00024, // fraction-of-path per ms
        born: now,
    ...
    function draw(now) {
      ...
      if (!reduced) {
        ...
        if (now - lastSpawn > 280) {
          if (pulses.length < 18) spawnPulse(now);
          lastSpawn = now;
        }
      }
      ...
      grad.addColorStop(0, 'rgba(122,193,67,0.16)');
      grad.addColorStop(0.4, 'rgba(122,193,67,0.06)');
      grad.addColorStop(1, 'rgba(122,193,67,0)');
      ctx.fillStyle = grad;
      ctx.fillRect(0, 0, W, H);

      // Horizontal scan line — very subtle
      const scanY = ((elapsed * 0.04) % (H + 200)) - 100;
      const sgrad = ctx.createLinearGradient(0, scanY - 80, 0, scanY + 80);
      sgrad.addColorStop(0, 'rgba(255,255,255,0)');
      sgrad.addColorStop(0.5, 'rgba(154,224,86,0.04)');
    ...
    function onResize() {
      ...
      for (let i = 0; i < 6; i++) spawnPulse(now);
      raf = requestAnimationFrame(draw);
    }
